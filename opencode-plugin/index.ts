...
// MARUT_CONFIG and MARUT_LOG are required — set them in your shell profile.
// MARUT_ARGS is an optional escape hatch for extra flags (e.g. --sim, --agent-id).
// Keep MARUT_ARGS to short flags only — no paths with spaces.
const MARUT_CONFIG = process.env.MARUT_CONFIG ?? "";
const MARUT_LOG = process.env.MARUT_LOG ?? "";
const MARUT_EXTRA = (process.env.MARUT_ARGS ?? "").trim().split(/\s+/).filter(Boolean);
const MARUT_ARGS = [
  "--platform", "opencode",
  "--mode", "validate",
  ...(MARUT_CONFIG ? ["--config", MARUT_CONFIG] : []),
  ...(MARUT_LOG ? ["--log", MARUT_LOG] : []),
  ...MARUT_EXTRA,
];
